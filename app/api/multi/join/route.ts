...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!roomId || !name?.trim() || !deviceId || deviceId !== headerDeviceId) {
    ...
    const result = await mutateRoom<MultiRoomState | JoinFailure>(roomId, async (room) => {
      ...
      if (already) {
        if (!(await authenticatePlayer(req, roomId, room))) {
          return { error: 'Credencial de reconexion requerida.', status: 401 };
        }
    ...
    const credential = await issuePlayerCredential(result.roomId, deviceId);
    const response = NextResponse.json({ room: result, credential });
