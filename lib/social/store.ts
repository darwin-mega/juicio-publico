...
export async function createRoomInvites(roomId: string, inviteeIds: string[]) {
  ...
  const uniqueIds = Array.from(new Set(inviteeIds))
    .filter((id) => id !== me.user_id && isUuid(id) && friendIds.has(id))
    .slice(0, 50);
...
export async function createCompetition(name: string, durationType: 'monthly' | 'bimonthly', inviteeIds: string[]) {
  ...
  const memberIds = [
    me.user_id,
    ...Array.from(new Set(inviteeIds)).filter((id) => isUuid(id) && friendIds.has(id)).slice(0, 50),
  ];
