...
export function toggleMute() {
  ...
  // Usamos una rampa suave para evitar el "clic" audible
...
function playBuffer(buffer: AudioBuffer, vol = 0.5) {
  ...
  src.buffer = buffer;
  src.connect(gain);
  gain.connect(masterGain); // Conexión a Master
...
export async function startBackgroundMusic() {
  ...
  bgMusicGain.connect(masterGain); // Conexión a Master
  
  bgMusicGain.gain.setValueAtTime(BG_MUSIC_VOL, c.currentTime);
...
function tone(
  ...
  const gain = c.createGain();
  osc.connect(gain); gain.connect(masterGain); // Conexión a Master
  osc.type = type;
...
export async function playNewsJingle() {
  ...
  // Si la música de fondo está sonando, solo subimos el volumen temporalmente
  if (bgMusicGain) {
    const now = c.currentTime;
    bgMusicGain.gain.cancelScheduledValues(now);
    bgMusicGain.gain.exponentialRampToValueAtTime(0.6, now + 0.1);
    bgMusicGain.gain.exponentialRampToValueAtTime(BG_MUSIC_VOL, now + 3.5);
    return;
  }

  // Si no hay música de fondo, play normal
  const buffer = await loadFile('/Music/noticias.mp3');
  if (buffer) {
